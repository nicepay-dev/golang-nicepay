...
import (
	...
	snap "github.com/nicepay-dev/golang-nicepay/snap"
)
...
func SetApiConfig() {
	...
	snapVersion.ApiConfig.SetConfiguration(config)

}

func GetAccessToken() (*snap.ResponseAccessToken, error) {
	...
	return snapVersion.RequestSnapAccessToken(parameter)
...
func TestRequestSnapTransaction(t *testing.T) {
	...
	httpMethod := "POST"
	responseTransaction, err := snapVersion.RequestSnapTransaction(parameterRegisterVA, endPoint, accessToken, httpMethod)
	if err != nil {
...
func TestCancelVASnapTransaction(t *testing.T) {
	...
	responseTransaction, err := snapVersion.RequestSnapTransaction(parameterDelete, endPoint, accessToken, httpMethod)
...
func TestVerifySHA256RSA(t *testing.T) {
	...
	isVerified, error := snapVersion.Helper.VerifySHA256RSA(dataString, publicKeyString, signatureString)
